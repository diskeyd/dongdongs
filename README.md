...
- 빨간 `blocked` 표시가 있는 줄은 승인할 수 없습니다. 그 쪽의 DRAFT를 자동으로 지우지 못했다는 뜻이니 아래 "보내는 방법"대로 알려 주세요. 나머지 쪽은 정상 처리됩니다.
...
| 4 | **"(추가)" 그래프 쪽 1개**만 | 마지막 그래프 쪽 뒤에 새 쪽이 생겼는지, 쪽 수가 1 늘었는지 |
| 5 | 4까지 다 되면 **전부 승인** | 전체 결과 |
