...
def _content_end(texts: list[str], rules: dict, last_start: int, page_count: int, offset: int) -> tuple[int, str]:
    wanted = {squash(t) for t in rules.get("end_titles", [])}
    found: list[int] = []
    for text in texts:
        lines = _lines(text)
        for i, line in enumerate(lines[:-1]):
            if squash(line) in wanted and lines[i + 1].isdigit():
...
def read_test_index(pdf_path: Path, rules: dict | None) -> dict:
    ...
    for index, text in enumerate(texts):
        if squash(title) not in squash(text):
            continue
        lines = _lines(text)
        entries = []
...
__all__ = ["read_test_index", "squash"]
